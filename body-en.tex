...
contrapuntal nature, like many of Brahms's compositions \nota{a
  citação de brahms é estranha. acho melhor remover}. Every harmonic
analysis of such passage is nonsensical, and, if automatically
performed, will probably find cadences where there are none, instead
of detecting the structure of the piece.  Another important problem is
how to represent, internally, a roman numeral functional analysis. We
will make clear in the remainder of this paper how we approached these
problems. \nota{Este período pode ir para a parte em que falamos do
  que fazemos no artigo}
...
some concluding remarks and directions for future work. \nota{É
  necessário falar o que tem em cada seção em um artigo tão pequeno?
  Não podemos falar de tudo que fazemos sem dizer 'na seção a ou b'?}
...
without a third apart from a minor chord without a
third). \nota{Sugestão: substituir parênteses por vírgula. vide
  marie-claire}
...
surrounding passage) \nota{tirar parênteses}. Clearly, there might be
...
given chord is. \nota{Não entendi o último período após ler 4x}
...
\nota{eu acho que voce esta confundindo tipo de acorde com funcao
  tonal aqui com campo harmonico. o campo harmonico basico de uma
  tonalidade é construido pelas triades em cada nota da escala, esse
  campo harmonico pode ser extendido por acordes de emprestimo modal
  (como uma sexta napolitana) ou outra sonoridade. (alguns teoricos
  consideram o acorde de 6a aumentada como um acorde de grau VI em tom
  menor com a sexta aumentada acrescentada.) e eu acho bom refrasear
  ``defining what are the possible roman functions''; qual o objetivo?
  detectar textbook roman functions (um conjunto ``pequeno'') or
  reason about the possible roman functions? (um conjunto maior de
  possibilidades) e deixar essas coisas claras no texto. (podemos
  conversar no irc/skype se for necessario)}
...
defining what are the possible roman functions is not an easy
problem. For example, one might want to consider Naepolitan sixths as
a roman function, or a german augmented sixth chord, even if neither
of these is technically built from triads in the given key. Also,
important notes in a tonal function can be ommitted from the passage
having that function, and this is often done purposefully by the
compose to enrich the passage \cite{schoenberg83:theory}.
...
notes sounding at a given instant of time)\nota{tirar
  parênteses}. Therefore, we chose the format for the roman numeral
functional analysis to be a list of instantaneous analyses, where an
instantaneous analysis is a snapshot of the analysis for one sonority
of a song. Due to the key granularity problem described in section
...
harmonies \nota{and textures} such as the Bach chorales.
...
Rameau currently has implementations of four \nota{promete 4 e cita 3}
different roman numeral functional analysis; a hidden Markov model, a
k-nearest neighbors classifier and a trivial extension of Pardo \&
Birmingham's chord labeling algorithm \cite{pardo.ea99:automated}.
...
\begin{figure*}[t]
  \centering
  \includegraphics[trim=0cm 4cm 0cm 1.5cm]{exemplo-01}
