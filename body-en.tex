...
contrapuntal nature, like many of Brahms's compositions. \nota{para
  mim ``in the noise'' é ok, mas talvez seja melhor reescrever para o
  publico mais geral do icmc} Every harmonic analysis of such passage
...
numeral functional analysis. \nota{``We will make clear in the
  remainder if this paper how we approached these problems''} How we
...
harmonic analysis. Among the earliest ones are \nota{``Winograd's'',
  winograd and ulrich's da a impressão que fizeram juntos} Winograd
...
problems encountered on the path \nota{provavelmente ``along the way''
  é mais comum/correto que ``on the path''}.
...
\nota{agora está melhor? sim, talvez seja melhor falar mais na 1a
  frase, i.e. qual o problema em achar a tonalidade? qual o problema
  em detectar o numeral romano?}

\subsection{Key finding}
\label{sec:key-finding}

\nota{key finding é algo terrivelmente ambíguo, pode ter vários níveis
  de verdade. é tão ambiguo quanto ter varios escopos aninhados em um
...
\nota{``One of the goal of the Rameau project is to understand and
  compare''} One of the goal of the Rameau project is understanding and
...
of the algorithms \nota{citar monografia? sim!}. The analysis results can
...
model. Figure \ref{fig:amostra} shows such a sample. \nota{``As we can
  see''} As it can be seen, the Markov model has no concept of voice
...
\nota{eu suponho que voce queira dizer infty e nao inf}
...
\nota{voce é o literário do grupo, mas eu acho ``rameau is
  promissory'' estranho, especialmente porque promissory tambem é
  promissório como em ``nota promissória''} While still preliminary,
...
\nota{``is'' ja que o resto esta entre parenteses} are available at
\url{http://genos.mus.br/rameau/}.
